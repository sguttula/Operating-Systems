...
/*
A flat filesystem that keeps track of files in a single directory (table). The
filesystem should provide operations such as: initialize the filesystem, create a file, read the data from a file, write
a file with given data, append data to a file, remove a file, etc..
*/
...
//Print the file detail for a given index. 
void fDetail(int v)
{
	printf("File Name : %s\n",fTab[v].fname);
...
//initializes all the values and formats the disk
int fFormat(){
	resetfFat();
	resetfTab();
	return 0;
}
//Creates a file for the given file name by choosing a free block based on the existence of the file.It takes the input parameter as filename and returns Error or success
...
//Deletes the given file name from the start block based on the existence of the file is empty.It takes the input parameter as filename and returns Error or success
...
//Lists the files based on the parameter 0 or 1. it takes the input parameter as 0 or 1
...
int changeDir(char* name){
 ...
 //deletes the given directory name from the startblock
...
int main(int argc,char *argv[])
{
  ...
  //Getting the number of tracks and sectors from dserver
   ...
   // fserver as a server to fclient
